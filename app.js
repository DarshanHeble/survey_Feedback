    ...
    node.onclick = () => {
      ...
      if (step === 0) {
        ...
        if (!userResponses.consent) {
          alert('You must read and agree to the Informed Research Consent form before accessing the survey questions.');
          goToSection('sectionConsent');
          return;
        }
...
function acceptConsent() {
  ...
  if (!agreed || !agreed.checked) {
    if (declined && declined.checked) {
      alert('You have declined to participate. Access to the survey is disabled.');
    } else {
      alert('Please select "I agree to participate" to unlock and fill out the survey.');
    }
    return;
  }
...
function goToSection(sectionId) {
  // Strict Consent Enforcement: Block access to any survey section if consent is not granted
  ...
  if (sectionId === 'sectionConsent') updateStepper(0, 'Informed Research Consent');
  else if (sectionId === 'sectionA') updateStepper(1, 'Section A: Participant Info');
  else if (sectionId === 'sectionB') updateStepper(2, 'Section B: AI Familiarity');
  else if (sectionId === 'sectionC') updateStepper(3, `Question ${activeSnippetIndex + 1} of 5`);
  else if (sectionId === 'sectionD') updateStepper(4, 'Section D: Perception & Feedback');
  else if (sectionId === 'resultsDashboard') updateStepper(5, 'Section E: Results & Analytics');
}
...
function startSnippetTasks() {
  ...
  if (typeof SNIPPETS_DATA !== 'undefined') {
    activeTaskSnippets = SNIPPETS_DATA.filter(s => s.language.toLowerCase() === selectedLang.toLowerCase());
...
function loadSnippetQuestion(index) {
  ...
  // Update Meta Header
  document.getElementById('snippetCounterBadge').textContent = `Question ${index + 1} of 5`;
  ...
  document.getElementById('taskQuestionTitle').textContent = `Question ${index + 1}: ${item.language} Code Classification`;
  document.getElementById('problemStatementText').textContent = item.problem_statement;
  ...
  // Reset inputs
  document.querySelectorAll('input[name="which_human"]').forEach(r => r.checked = false);
  document.querySelectorAll('input[name="current_q_confidence"]').forEach(r => r.checked = false);

  if (existing) {
    if (existing.userChoice) {
      const radioChoice = form.querySelector(`input[name="which_human"][value="${existing.userChoice}"]`);
      if (radioChoice) radioChoice.checked = true;
    }
    if (existing.confidence) {
      const radioConf = form.querySelector(`input[name="current_q_confidence"][value="${existing.confidence}"]`);
...
function saveCurrentSnippetData() {
  ...
  const selectedChoice = form.querySelector('input[name="which_human"]:checked');
  const selectedConf = form.querySelector('input[name="current_q_confidence"]:checked');
...
function updateSnippetDots() {
  ...
  dots.forEach((dot, idx) => {
    ...
    if (userResponses.snippetAnswers[snipId] && userResponses.snippetAnswers[snipId].userChoice) {
...
function submitSurvey() {
  ...
  activeTaskSnippets.forEach((item, idx) => {
    const ans = userResponses.snippetAnswers[item.id] || { userChoice: 'Unanswered', confidence: '3' };
  ...
  document.getElementById('scoreCirclePath').setAttribute('stroke-dasharray', `${accuracyPct}, 100`);
  document.getElementById('scoreHeadline').textContent = `Calculated Accuracy: ${correctCount} / ${totalEvaluated} Correct Decisions`;
  ...
  document.getElementById('scoreConfidencePill').textContent = `Avg Confidence: ${avgConf} / 5`;
